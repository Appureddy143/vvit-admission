<?php
// Set your timezone to make the date() function reliable
date_default_timezone_set('Asia/Kolkata');

// --- NEW: Include the Twilio SDK (from composer) ---
...
/**
 * --- NEW: Sends an SMS using the Twilio API ---
 * It securely gets the credentials from the environment.
 */
function sendSMSMessage($studentMobile, $messageBody) {
    ...
    if (!$sid || !$token || !$twilioNumber) {
        error_log("Twilio credentials are not set in environment variables.");
        return false;
    }
    ...
    try {
        ...
        $client->messages->create(
            $studentMobile, // The 'to' number (e.g., +919876543210)
            [
                'from' => $twilioNumber, // Your Twilio 'from' number
                'body' => $messageBody
            ]
        );
        return true;
...
/**
 * Generates the unique student ID.
 * --- FIX: Removed slow external API call for time ---
 */
function generateUniqueCode($pdo) {
    $collegeCode = "1VJ";
    $year = date("y"); // Uses the timezone set at the top
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // --- SECURITY FIX: Validate file uploads ---
    ...
    foreach ($_FILES as $key => $file) {
        if (isset($file['error']) && $file['error'] === UPLOAD_ERR_OK) {
            ...
            if ($file['size'] > $maxFileSize) {
                $uploadErrors[] = "Error: File '{$file['name']}' is too large (Max 5MB).";
                continue;
            }
            // Check file type
            if (!in_array($fileExtension, $allowedExtensions)) {
                $uploadErrors[] = "Error: File '{$file['name']}' has an invalid type (Allowed: jpg, png, pdf).";
                continue;
            }
            ...
            if (move_uploaded_file($file['tmp_name'], $targetPath)) {
                // IMPORTANT: On Render, these files will be deleted.
                // You must use a service like Amazon S3 for permanent storage.
                $fileUrls[$key . '_url'] = $targetPath;
            } else {
                 $uploadErrors[] = "Error moving uploaded file '{$file['name']}'.";
            }
        }
    }
    ...
    try {
        ...
        if (!empty($sanitized_post['mobile_number'])) {
            // Clean number and add country code (e.g., +91 for India)
            // !! UPDATE THIS to your country code if it's not India !!
            $studentMobile = '+91' . preg_replace('/[^0-9]/', '', $sanitized_post['mobile_number']);
            
            // Create the message
            $messageBody = "Welcome, {$sanitized_post['student_name']}. Your Admission ID for Vijay Vittal Institute is: {$studentId}";
            
            // Send the SMS
            sendSMSMessage($studentMobile, $messageBody);
        }
        
        // --- UPDATED: Success Modal ---
        // (Changed note from WhatsApp to SMS)
        echo <<<HTML
        <style>
        ...
        <div class="success-modal">
            ...
            <p class="note">A confirmation SMS has been sent to your mobile number. Please save this ID for future reference.</p>
        </div>
        <script>
 ...
 <a href="/admin/php" class="header-btn">College Details</a>
    ...
    <div class="container">
        ...
        <form action="index.php" method="POST" enctype="multipart/form-data">
            ...
            <label for="mobile_number">Mobile Number (e.g., 9876543210)</label> <input type="tel" id="mobile_number" name="mobile_number" required>
            ...
            <label for="photo">Passport Size Photo (Max 5MB: JPG, PNG)</label> <input type="file" id="photo" name="photo" required>
            <label for="marks_card">Previous Marks Card (Max 5MB: PDF, JPG, PNG)</label> <input type="file" id="marks_card" name="marks_card" required>
            <label for="aadhaar_front">Aadhaar Card (Front) (Max 5MB: JPG, PNG)</label> <input type="file" id="aadhaar_front" name="aadhaar_front" required>
            <label for="aadhaar_back">Aadhaar Card (Back) (Max 5MB: JPG, PNG)</label> <input type="file" id="aadhaar_back" name="aadhaar_back" required>
            <div id="casteIncomeSection"> <label for="caste_income">Caste & Income Certificate (if applicable) (Max 5MB: PDF, JPG, PNG)</label> <input type="file" id="caste_income" name="caste_income"> </div>
            <button type="submit">Submit Application</button>
        </form>
    </div>
    <script>
        // Your Javascript for toggling fields remains the same
